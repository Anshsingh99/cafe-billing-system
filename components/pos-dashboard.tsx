...
export function PosDashboard() {
  ...
  async function load() {
    ...
    if (!response.ok) {
      setMessage(json.error || "Unable to load POS data");
      return;
    }
    setProducts(
    ...
    try {
      saved = JSON.parse(window.localStorage.getItem(cartStorageKey) || "{}");
    } catch {
      saved = {};
    }
    const nextTables = (json.tables ?? []).map((t: Table) => ({
      ...t,
      items: saved[t.id] ?? ([] as LineItem[]),
    }));
    let pendingIds: string[] = [];
    try {
      pendingIds = JSON.parse(
        window.localStorage.getItem(pendingTablesKey) || "[]",
      );
    } catch {
      pendingIds = [];
    }
    const mergedTables = nextTables.map((table) =>
      pendingIds.includes(table.id) && table.items.length
        ? { ...table, status: "occupied" }
        : table,
    );
    setTables(mergedTables);
    setSelectedId((current) =>
      current && nextTables.some((t) => t.id === current)
        ? current
        : (nextTables[0]?.id ?? ""),
    );
    setGst(Number(json.settings?.gst_percentage ?? 5));
  ...
  useEffect(() => {
    if (tables.length) {
      window.localStorage.setItem(
        cartStorageKey,
        JSON.stringify(
          Object.fromEntries(tables.map((table) => [table.id, table.items])),
        ),
      );
      window.localStorage.setItem(
        pendingTablesKey,
        JSON.stringify(
          tables.filter((table) => table.items.length).map((table) => table.id),
        ),
      );
    }
  }, [tables]);
    ...
    setTables((all) =>
      all.map((table) => {
        ...
        const items = existing
          ...
          : [...table.items, { ...product, quantity: count }];
        return { ...table, items, status: "occupied" };
      }),
    );
    setQuery("");
    setQuantity("1");
  ...
  async function addTable() {
    ...
    try {
      ...
      setMessage("Network error while adding table");
      return;
    }
    ...
    if (!response.ok) {
      setMessage(json.error || `Unable to add table (${response.status})`);
      return;
    }
    if (json.table) {
      setTables((all) => [...all, { ...json.table, items: [] }]);
      setSelectedId(json.table.id);
      setMessage(`Table ${json.table.table_number} added successfully`);
    }
  }
  async function deleteTable(id: string) {
    ...
    if (!response.ok) {
      const json = await response.json();
      setMessage(json.error || "Unable to delete table");
      return;
    }
    setTables((all) => all.filter((table) => table.id !== id));
    setMessage("Table deleted");
    if (selectedId === id)
      setSelectedId(tables.find((table) => table.id !== id)?.id ?? "");
  }
  async function saveBill() {
    ...
    const json = await response.json();
    if (!response.ok) {
      setMessage(json.error || "Unable to save bill");
      return;
    }
    setPaid(true);
    setMessage("Bill saved to history");
  }

  function shareWhatsApp() {
    console.log("🔥 NEW WHATSAPP FUNCTION RUNNING - VERSION 2026-08-20");
    ...
    const billNumber = `UC-${Date.now().toString().slice(-6)}`;
    ...
    const gstLine = gst > 0 ? `*GST (${gst}%):* ₹${gstAmount}` : "";

    const messageText = [
      "*☕ ULLAS CAFE & DINE* helo",
      ...
      <main className="min-h-screen lg:pl-72">
        <header className="flex h-20 items-center border-b border-border bg-card px-5 md:px-8">
          ...
          <div>
            ...
            <h1 className="font-serif text-2xl font-bold">Table orders</h1>
          </div>
        </header>
        <div className="mx-auto max-w-[1500px] p-5 md:p-8">
          {message && (
            <p className="mb-5 rounded-xl bg-secondary px-4 py-3 text-sm">
              {message}
            </p>
          )}
          <div className="mb-7 flex items-center justify-between">
            <div>
              <h2 className="font-serif text-2xl font-bold">Tables</h2>
              <p className="text-sm text-muted-foreground">
                Select a table, add items, and make a bill.
              </p>
            </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {tables.map((table) => (
              <div
                key={table.id}
                className={`rounded-2xl border bg-card p-5 ${selectedId === table.id ? "border-primary ring-2 ring-primary/15" : "border-border"}`}
              >
                <button
                ...
                >
                  ...
                  <p className="text-sm text-muted-foreground">
                    {table.items.length} items · {table.status}
                  </p>
                </button>
          ...
          <div className="mt-8 grid gap-6 xl:grid-cols-[1fr_360px]">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h2 className="font-serif text-xl font-bold">
                    Table {current?.table_number ?? "—"} menu
                  </h2>
                  <p className="text-sm text-muted-foreground">
                    Search and add items to this table.
                  </p>
                </div>
                <div className="flex w-full gap-2 sm:w-auto">
                  <input
                    ...
                    className="h-11 min-w-0 flex-1 rounded-xl border border-input bg-background px-4 text-sm sm:w-56"
                  />
                  <input
                    value={quantity}
                    ...
                    type="number"
                    min="1"
                    className="h-11 w-20 rounded-xl border border-input bg-background px-3 text-sm"
                  />
                </div>
              </div>
              <div className="mt-5 flex max-h-96 flex-wrap content-start gap-2 overflow-auto">
                {filtered.map((product) => (
                  <button
                    key={product.id}
                    className="rounded-xl border border-border px-3 py-2 text-left hover:border-primary"
            ...
            <aside className="rounded-2xl border border-border bg-card p-5">
              <div className="flex items-center justify-between">
                <h2 className="font-serif text-xl font-bold">Current bill</h2>
          ...
          <div className="w-full max-w-md rounded-2xl bg-card p-6 shadow-xl">
            {paid ? (
              ...
              <>
                ...
                <Button className="mt-5 w-full" onClick={() => void saveBill()}>
