...
export async function PUT(request: Request) {
  try {
    ...
    const gstPercentage = Number(body.gst_percentage ?? 5); const discountPercentage = Math.min(100, Math.max(0, Number(body.discount_percentage) || 0)); const discountAmount = Math.round(subtotal * discountPercentage / 100); const taxableAmount = subtotal - discountAmount; const gstAmount = Math.round(taxableAmount * gstPercentage / 100); const total = taxableAmount + gstAmount
    const { data: order, error: orderError } = await db.from('orders').insert({ table_id: body.table_id, status: 'completed', subtotal, gst_percentage: gstPercentage, discount_percentage: discountPercentage, discount_amount: discountAmount, gst_amount: gstAmount, total, customer_name: String(body.customer_name || ''), customer_phone: String(body.customer_phone || ''), completed_at: new Date().toISOString() }).select().single()
